	...
	<div class="px-3 max-h-64 overflow-y-auto scrollbar-hidden group relative">
		...
		{#each filteredItems as item, index}
			<button
				...
				class="flex w-full text-left font-medium line-clamp-1 select-none items-center rounded-button py-2 pl-3 pr-1.5 text-sm text-gray-700 dark:text-gray-100 outline-hidden transition-all duration-75 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg cursor-pointer data-highlighted:bg-muted {showSelectedModel &&
				index === selectedModelIdx
					? 'bg-gray-100 dark:bg-gray-800 group-hover:bg-transparent'
					: ''}"
				data-arrow-selected={showSelectedModel && index === selectedModelIdx}
			...
			>
				...
				{#if showSelectedModel && value === item.value}
